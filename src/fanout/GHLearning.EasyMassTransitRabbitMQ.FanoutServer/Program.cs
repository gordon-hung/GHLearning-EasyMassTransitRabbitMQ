...
using GHLearning.EasyMassTransitRabbitMQ.FanoutServer.ConsumerHeaders;
using MassTransit;
...
builder.Services.AddMassTransit(registrationConfigurator =>
{
	...
	registrationConfigurator.UsingRabbitMq((context, configurator) =>
	{
		...
		var exchangeName = $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Fanout";

		configurator.Message<LoginMessage>(e => e.SetEntityName(exchangeName)); // name of the primary exchange
		configurator.Publish<LoginMessage>(e =>
		{
			e.Durable = true; // default: true
			e.AutoDelete = false; // default: false
			e.ExchangeType = ExchangeType.Fanout;
		});
		configurator.Send<LoginMessage>(e =>
			// multiple conventions can be set, in this case also CorrelationId
			e.UseCorrelationId(context => context.Id));

		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Fanout.Login.Alert",
			configureEndpoint: endpointConfigurator =>
			{
				...
				endpointConfigurator.Consumer<LoginAlertConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s => s.ExchangeType = ExchangeType.Fanout);
			});

		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Fanout.Login.Record",
			configureEndpoint: endpointConfigurator =>
			{
				endpointConfigurator.ConfigureConsumeTopology = false;
				endpointConfigurator.SetQuorumQueue();
				endpointConfigurator.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
				endpointConfigurator.Consumer<LoginRecordConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s => s.ExchangeType = ExchangeType.Fanout);
