...
builder.Services.AddMassTransit(registrationConfigurator =>
{
	...
	registrationConfigurator.UsingRabbitMq((context, configurator) =>
	{
		...
		var exchangeName = $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Direct";
		...
		configurator.Send<OrderMessage>(e =>
		{
			...
			e.UseRoutingKeyFormatter(context => context.Message.Status.ToString());
		...
		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Direct.Order.{OrderStatus.Pending}",
			configureEndpoint: endpointConfigurator =>
			{
				...
				endpointConfigurator.Consumer<OrderPendingConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s =>
					{
						s.RoutingKey = OrderStatus.Pending.ToString();
						s.ExchangeType = ExchangeType.Direct;
					});
			});

		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Direct.Order.{OrderStatus.Processing}",
			configureEndpoint: endpointConfigurator =>
			{
				endpointConfigurator.ConfigureConsumeTopology = false;
				endpointConfigurator.SetQuorumQueue();
				endpointConfigurator.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
				endpointConfigurator.Consumer<OrderProcessingConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s =>
					{
						s.RoutingKey = OrderStatus.Processing.ToString();
						s.ExchangeType = ExchangeType.Direct;
					});
			});

		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Direct.Order.{OrderStatus.Shipped}",
			configureEndpoint: endpointConfigurator =>
			{
				endpointConfigurator.ConfigureConsumeTopology = false;
				endpointConfigurator.SetQuorumQueue();
				endpointConfigurator.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
				endpointConfigurator.Consumer<OrderShippedConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s =>
					{
						s.RoutingKey = OrderStatus.Shipped.ToString();
						s.ExchangeType = ExchangeType.Direct;
					});
			});

		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Direct.Order.{OrderStatus.Delivered}",
			configureEndpoint: endpointConfigurator =>
			{
				endpointConfigurator.ConfigureConsumeTopology = false;
				endpointConfigurator.SetQuorumQueue();
				endpointConfigurator.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
				endpointConfigurator.Consumer<OrderDeliveredConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s =>
					{
						s.RoutingKey = OrderStatus.Delivered.ToString();
						s.ExchangeType = ExchangeType.Direct;
					});
			});

		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Direct.Order.{OrderStatus.Completed}",
			configureEndpoint: endpointConfigurator =>
			{
				endpointConfigurator.ConfigureConsumeTopology = false;
				endpointConfigurator.SetQuorumQueue();
				endpointConfigurator.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
				endpointConfigurator.Consumer<OrderCompletedConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s =>
					{
						s.RoutingKey = OrderStatus.Completed.ToString();
						s.ExchangeType = ExchangeType.Direct;
					});
			});
	});
