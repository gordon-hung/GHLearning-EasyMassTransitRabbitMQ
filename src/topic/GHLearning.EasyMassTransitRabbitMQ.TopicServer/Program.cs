...
using GHLearning.EasyMassTransitRabbitMQ.TopicServer.ConsumerHeaders;
using MassTransit;
...
builder.Services.AddMassTransit(registrationConfigurator =>
{
	...
	registrationConfigurator.UsingRabbitMq((context, configurator) =>
	{
		...
		var exchangeName = $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Topic";

		configurator.Message<NotificationMessage>(e => e.SetEntityName(exchangeName)); // name of the primary exchange
		configurator.Publish<NotificationMessage>(e =>
		{
			e.Durable = true; // default: true
			e.AutoDelete = false; // default: false
			e.ExchangeType = ExchangeType.Topic;
		});
		configurator.Send<NotificationMessage>(e =>
		{
			// use customerType for the routing key
			e.UseRoutingKeyFormatter(context => $"Notification.{context.Message.Medium}");
		...
		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Topic.Notification.{NotificationType.Public}",
			configureEndpoint: endpointConfigurator =>
			{
				...
				endpointConfigurator.Consumer<NotificationPublicConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s =>
					{
						s.RoutingKey = $"{NotificationType.Public}.*";
						s.ExchangeType = ExchangeType.Topic;
					});
			});

		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Topic.Notification.{NotificationType.Group}",
			configureEndpoint: endpointConfigurator =>
			{
				endpointConfigurator.ConfigureConsumeTopology = false;
				endpointConfigurator.SetQuorumQueue();
				endpointConfigurator.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
				endpointConfigurator.Consumer<NotificationGroupConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s =>
					{
						s.RoutingKey = $"{NotificationType.Group}.*";
						s.ExchangeType = ExchangeType.Topic;
					});
			});

		configurator.ReceiveEndpoint(
			queueName: $"{builder.Environment.EnvironmentName}.GHLearning.EasyMassTransitRabbitMQ.Topic.Notification.{NotificationType.Personal}",
			configureEndpoint: endpointConfigurator =>
			{
				endpointConfigurator.ConfigureConsumeTopology = false;
				endpointConfigurator.SetQuorumQueue();
				endpointConfigurator.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
				endpointConfigurator.Consumer<NotificationPersonalConsumerHeader>(context);
				endpointConfigurator.DiscardFaultedMessages();
				endpointConfigurator.Bind(
					exchangeName: exchangeName,
					callback: s =>
					{
						s.RoutingKey = $"{NotificationType.Personal}.*";
						s.ExchangeType = ExchangeType.Topic;
					});
			});
	});
